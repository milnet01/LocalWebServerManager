...
Every version below traces to a numbered success criterion in
[`docs/discovery.md § Success criteria`](docs/discovery.md), so
nothing ships that nobody asked for and no criterion goes
unbuilt.

Until 2026-09-25 this roadmap was grouped by phase and fold-in
(P01–P10, FP01–FP09, DS01). Those section intros are kept in
[`docs/roadmap/phase-sections-2026-09-25.md`](docs/roadmap/phase-sections-2026-09-25.md).
Commit prefixes and item headlines still use the phase IDs.

| Criterion | Version that delivers it |
| --- | --- |
| 1 — zero-config first run finds all seven projects | 0.1.0 ships the scanner (LWSM-1006, 1007, 1131); 0.2.0 finishes it (LWSM-1039, 1008, 1121) |
| 2 — start / restart / stop round-trip within 2 s, reachable in a browser | 0.1.0 (LWSM-1009, 1010, 1016); 0.2.0 adds service-managed projects (LWSM-1028) |
| 3 — truthful status across app restarts and foreign servers | 0.3.0 |
| 4 — port conflicts caught before launch, reassignment persists | 0.4.0 |
| 5 — failures readable without a terminal | 0.5.0 |

## 0.1.0 — Looks finished

The app as it stands, made solid: the FP09 review fixes, the FP01 security
...
- 📋 [LWSM-1312] **roadmap-format.md § 3.7 still requires a target date that no version heading carries.**
  ...
  in a separate divergence row: the date is optional.
  Not copied under LWSM-1311, because global rule 14's write-back exception
...
- ✅ [LWSM-1195] **Two `###` subsections holding FP07 items render outside FP07.**
  ...
  **It cannot be fixed with the roadmap verbs as they stand.** `roadmap_log`
  ...
  Resolved (2026-09-25) by LWSM-1311: both misplaced `###` blocks were
  deleted when the roadmap moved to version sections, and ROADMAP.md now
  carries no `###` heading at all. roadmap_log has since gained
  delete_section, move_section and amend_field field:"section", which
  this bullet was waiting for.
...
- ✅ [LWSM-1311] **Organise the roadmap by version instead of by phase.**
  ...
  Done (2026-09-25). Seven version sections created; every item moved by
  amend_field (open items first, then shipped by id); all 21 phase,
  fold-in and theme sections deleted; the coverage table rebuilt with a
  Version column; the preamble updated. Checked by comparing every
  item's text before and after: all present, none changed.
  docs/standards/roadmap-format.md § 3.2 now follows the global CFG-0321
  rule. Two leftovers filed separately: § 3.7's target date, and the
  legend's "this phase" wording.
...
buttons already shipped in 0.1.0. What is left: the first-run confirmation,
a registry backup, the extra port sources, service-managed projects, and two
...
[ADR-0002](docs/decisions/0002-port-contract.md) is the contract.
...
## 1.0.0 — A download for other people

The project is public and meant to be useful to strangers, so a release has to
be something a person downloads and runs. Nothing here is needed for the
author's own use, which is why it sits after the app works.
...
open DS01 debt-sweep items, and the open FP02 review items.
