...
**One carve-out to "never edited after acceptance."** Corrections
arising from an ADR's own review gate — the `/cold-eyes` pass
that global rule 14 requires before anything is built from it —
are applied **in place**, before the first implementation. The
"never edited" rule protects decisions that code already
embodies; an ADR reviewed on the day it was written embodies
nothing yet, and superseding it with a near-identical file would
add ceremony without preserving any history worth keeping. Once
implementation begins, the rule applies in full: an accepted ADR
