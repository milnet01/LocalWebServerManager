...
- Opened `O_RDONLY | O_NONBLOCK | O_NOFOLLOW`, then `fstat` on the raw
  ...
  - A FIFO at `start.sh` would block `open()` until a writer appears — the
    failure `configfile.py::read_bounded` was written for, reproduced there on
...
demands one link and our own ownership) and `configfile.py::read_bounded` (our
...
- **Extract `configfile.py::read_bounded` into a shared helper rather than
  ...
  **Partly reversed since, and not by this item.** LWSM-1031 needed a
  second config file and extracted the helper into
  `configfile.py::read_bounded`, which `registry.py` and `settings.py`
  now share. The decision recorded here still holds for the scanner:
  `scanner.py` keeps its own reader, on the policy differences above.
- **Keep the matched line on `PortFinding` for diagnosis, escaped and clipped
  through a `configfile.py::quoted` equivalent.** Rejected in favour of deleting
...
The columns differ because the severity scale was retired for a
per-question tally. A landed loop-log row is never rewritten, so the
log is split rather than restated.
