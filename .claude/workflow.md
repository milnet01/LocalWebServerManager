...
| **Last update** | 2026-09-06 (**Twelve items shipped and pushed, one commit each; tree clean, nothing unpushed.** Ten FP09 items — LWSM-1239, 1240, 1241, 1242, 1243 (the whole `placement.py` cluster except LWSM-1238), then 1245, 1246, 1247, 1249, 1250 — plus two filed and fixed from live user reports the same day, LWSM-1297 and LWSM-1298. FP09 went 45/46 to **55 shipped / 36 active**; read the live figure, never this one. `local-ci.sh` green throughout, 1500 tests at the end, no SKIP, no tool drift; every fix mutation-probed and the mutants recorded on its bullet.

**Two things are waiting on the USER, and neither is a code defect.** LWSM-1238 needs a screenshot of a focused button on the real desktop (see below). And the user's own `projects.json` is STALE relative to the scanner: measured 2026-09-06 against the live `/mnt/Games/Scripts/Linux` tree, a fresh scan finds Contact_List on **5002** (`config.py`) and RetroDB on **5000** (`settings_manager.py`) where the app shows "no port", while MAME_Curator shows 1024 that a fresh scan does not find — which is the shape of a user `port_override`, preserved across rescans by design. **Rescan should fix the first two; if it does not, that is a merge defect worth an item.** Recorded here because it was diagnosed in chat and lives in no file otherwise.

**LWSM-1238 is the one left OPEN in that cluster and it is BLOCKED on an observation, not on work.** It needs to be known what the real desktop style draws on a focused button today: offscreen Fusion draws nothing at all on `midnight`, and a focus ring costs geometry where LWSM-1298's pressed colour did not — any unconditional or `:focus` border switches a `QPushButton` to `QStyleSheetStyle` box sizing and drops its height hint from 25 to 21 at rest. A `:pressed` border does not. Measure before building.
