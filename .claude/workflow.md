...
| Field | Value |
|-------|-------|
| **Project phase** | **`P04` FEATURE-COMPLETE, close STILL BLOCKED by `FP09`, which is now part-closed (2026-09-02).** `FP09` is no longer "every item open" — read the live figure rather than any number written here, with `roadmap_query mode="section_index" query="fp09"`, which returns `active_count_id_only` and `shipped_count_id_only` in one small call and is the cheapest way to ask. At the last measurement on 2026-09-02 it stood at 35 shipped of 91. **`P04-complete` still waits on `FP09` reaching zero active**, unchanged, and it remains LWSM-1152's gate for the first 0.1.0 release. **`P03b` (LWSM-1039, LWSM-1008, LWSM-1121) REMAINS OPEN** and deferred behind P04 by decision. Items in `## Findings filed in passing` gate NOTHING — LWSM-1291–1294 joined that section on 2026-09-02 and gate nothing either. Was: **`P04` FEATURE-COMPLETE, close BLOCKED AGAIN by `FP09` (filed 2026-09-01).** The close's own steps 5-6 have run - a whole-tree `check-code` then a fourteen-lane `review-code` - and returned the `FP09` fold-in, every item open at filing. `P04-complete` therefore waits on `FP09` closing, exactly as it waited on `FP08`; it is LWSM-1152's gate for the first 0.1.0 release. `P03b` (LWSM-1039, LWSM-1008, LWSM-1121) REMAINS OPEN and deferred behind P04 by decision. Items in `## Findings filed in passing` gate NOTHING. Was: **`P04` FEATURE-COMPLETE and UNBLOCKED — `FP08` CLOSED 2026-08-31.** No FP08 item is still open (verified by `roadmap_query section=fp08-… status=active`, count 0). **`P04` has NOT earned `P04-complete` yet**: that tag comes from the close's own steps 5-9 (`/close-phase`), which have not run, and it is LWSM-1152's gate for the first 0.1.0 release. **`P03b` REMAINS OPEN** — LWSM-1039, LWSM-1008 and LWSM-1121 are still 📋, deferred behind P04 by decision. **Three items were filed in passing on 2026-08-31 and gate NOTHING** — LWSM-1192, LWSM-1193, LWSM-1194, in the new `## Findings filed in passing` section, which exists precisely so an item like these is not mistaken for an unclosed phase. Was: **`P04` FEATURE-COMPLETE, close BLOCKED by `FP08` (2026-08-21).** Every P04 item is ✅ — LWSM-1033 (window geometry, Centre on screen) and LWSM-1156 (Enter in the filter box) shipped today, closing the appearance/accessibility/keyboard/layout theme. The close then ran four cold lanes, which returned **20 verified findings** — filed as `FP08`, LWSM-1162…LWSM-1181. **P04 stays 🚧 until FP08 closes**, and only then earns `P04-complete`, which is also LWSM-1152's gate for the first 0.1.0 release. **`P03b` REMAINS OPEN and is untouched by this** — LWSM-1039, LWSM-1008 and LWSM-1121 are still 📋. Two phases are legitimately in flight; `CLAUDE.md § Commit conventions` already records that the COMMIT LOG's prefix, not this header, decides what a commit is labelled, and this line is the reconciliation that note said a close would have to make. Was: **`P03b` OPEN (started 2026-08-12)** — the continuation phase carrying the four items P03 planned and did not deliver: **LWSM-1007** (registry persistence, active), LWSM-1039 (backup), LWSM-1008 (first-run flow), LWSM-1121 (extra port sources). **Why a suffix and not `P04`:** this roadmap assigns `P04`–`P09` to named themes in advance (P04 theme/accessibility/keyboard, P05 supervisor, P06 classifier, …), so a phase closed against partial scope has no free number to spill into. Three alternatives were put to the user and declined — renumbering the themes (28 bullets plus every doc citing a phase by number), folding the leftovers into P04 (one phase mixing storage internals with UI polish), and re-opening P03 by re-pointing the pushed `P03-complete` tag (needs the force-push authorisation `commits.md § 4.2` withholds). The suffix form is now recorded in `CLAUDE.md § Commit conventions`; commits read `P03b: …` and the close earns its own `P03b-complete` tag. **`P03-complete` stands and is not re-pointed** — it marks what P03 actually closed. Was: **P03 CLOSED 2026-08-12** — LWSM-1006 Scanner shipped, plus fix-pass FP06 (nine items). Tagged `P03-complete`. **P03's remaining items — LWSM-1007 registry persistence, LWSM-1039 backup, LWSM-1008 first-run flow and LWSM-1121 — are 📋 and were never part of this close**; they open the next phase. Was: P03 OPEN (started 2026-08-08) — LWSM-1006 Scanner is the active item and its spec is through the gate; LWSM-1007, LWSM-1039, LWSM-1008 and LWSM-1121 follow. **P02 CLOSED 2026-08-07** — vertical slice LWSM-1005 plus five fix-passes (FP02–FP05, 37 fix items). P01 stays open only for FP01's five 🚧 security items (P05/P06). Phases A–D closed 2026-08-03. **Next is P03** (LWSM-1006 Scanner, LWSM-1007 registry persistence, LWSM-1039 backup, LWSM-1008 first-run flow) |
| **Active item ID** | **`FP09`. A calibrated-down finding — one the fold-in itself marks as another item's scope — is CLOSED as a finding rather than built here: flip it shipped, say plainly in the note that no code changed, and name where the work lives (user decision 2026-09-03, first applied to LWSM-1230, which routes to LWSM-1011). Do not re-open that; and do not read such a tick as the work having been done.** **The list is NOT being worked strictly in order (2026-09-02).** Items are picked where they group: LWSM-1253 was done immediately before LWSM-1254 because that item's document amendment would otherwise have written a promise the browser combo broke. **Two user decisions taken 2026-09-02 and recorded on the bullets themselves**, so do not re-open them: LWSM-1244 was to BUILD Follow system, both halves, rather than strike the promise; LWSM-1254 was to AMEND `design-accessibility.md` rather than stop the app eliding. **Shipped this session: LWSM-1225 (bookkeeping only — its fix had landed in `1cbc627` and was never flipped), LWSM-1244, LWSM-1253, LWSM-1254, LWSM-1229.** All pushed; tip `07ed551`. **Next**: any open `FP09` item; the HIGHs are done, so what remains is MEDIUM, the LOW batches LWSM-1271–1284, and the two INFO items. **Do NOT re-run `review-code`** — it cost ~2.8M tokens and the skill forbids re-running to chase a clean result. **Do not re-triage `.ants_review_falsepos.jsonl`.** LWSM-1230 and LWSM-1244 were deliberately calibrated HIGH→MEDIUM with the reason on the bullet; LWSM-1270 is a dead symbol and not a zombie feature. Was: **`FP09`, working the list in order.** **Four decisions taken by the user 2026-09-02, all recorded rather than left in chat.** (1) The first release is **`0.1.0`**, kept against the newly-adopted versioning standard's `0.0.1` and written into `docs/standards/versioning-overrides.md` § 3 as the single visible exception — the global standard says an overrides file carries answers, not deltas, so an override must be legible as one. (2) `Wedding_Site/start.sh` was `chmod g-w`'d: it was 0775 and the group-writable refusal correctly blocked it, so the app could not start one of the author's own projects; all six real launchers validate now. NOTE this wrote to the sibling tree, which `CLAUDE.md` otherwise says to treat as read-only. (3) **The empty `## Success-criteria coverage` section is filled AFTER the FP09 queue, not before** — it ties criteria 1-5 to the items that deliver them and is what makes the 1.0 exit condition checkable by someone other than the author. (4) LWSM-1288 is planned rather than considered, and carries a `LWSM-1131 § 4.3` spec amendment plus that spec's own gate. Was: **`FP09`, working the list in order — every HIGH plus the first nine MEDIUMs are closed (LWSM-1196…LWSM-1218), one commit each, gate green on every one.** **Measurement has now contradicted a bullet's stated cause or prescribed fix nine times**, so the standing rule keeps paying: LWSM-1197's HIGH security claim does not hold at all, LWSM-1217's premise is unreproducible (there is no depth where the load clears and the re-serialise fails — the element is a sub-tree of what the load accepted), LWSM-1215 and LWSM-1218 named mechanisms this file's own rules forbid, and LWSM-1214's "route to review-contract" is the No branch by rule 14's own named instance. **Five of my own tests were vacuous or backwards and a mutant or a reddened neighbour caught each** — a control string a slash split, a host that always matches its own hint, two assertions satisfied by unrelated prose, an absent field asserted on the wrong sibling, and a write-ordering comment that claimed the opposite of what the ordering does. **Watch for two traps recorded in the bullets**: `from dataclasses import MISSING` is shadowed by this project's own `MISSING = "missing"`, and `caplog` sees nothing after `applog.configure_logging()`. Was: **`FP09` — every HIGH is closed; the MEDIUMs remain.** LWSM-1196 through LWSM-1209 shipped 2026-09-02, one commit each, gate green on every one. **LWSM-1189 closed with them and not by its own diagnosis**: it blamed two tests for not stopping what they started, and the defect was `Supervisor.stop` enumerating the process group once — the fixture already called `stop()`, and `stop()` was what leaked. A guard fixture now fails the run and names the offender, matched by CWD rather than by command line. **Bullets whose stated cause or prescribed fix measurement changed: six of fourteen.** LWSM-1197's HIGH security claim does not hold at all (the controller already refuses a foreign stop, and a test has pinned it since LWSM-1032) — it is a UX defect. LWSM-1196 named two break characters that do not break and missed one that does. LWSM-1201 asks for a state that does not exist yet. LWSM-1203 says no signal is ever emitted, and the first-poll branch does emit. LWSM-1209's fix needed a second escaping layer the bullet did not mention. LWSM-1205 was a decision, taken toward implementing the ADR rather than amending it. **Three of my own tests were vacuous and mutants or a reddened neighbour caught each**: a control string holding a slash that `Path` split, a `widgetResizable` host that always matches its own hint, and two CONTRIBUTING assertions satisfied by unrelated prose. Was: **`FP09`, working the list in order.** Shipped 2026-09-02: **LWSM-1196** (`804c205`) - the trust prompt escaped CR and LF only, and Qt also breaks on U+2028 and U+2029. **Its bullet was wrong twice and measurement caught both**: U+0085 NEL does not break, and neither do VT or FF, so the fix the bullet prescribed would have been three dead branches plus a live hole. The bullet's "the suite cannot see this" is also wrong - comparing the rendered heights of two names differing by one character sees it headless, and being a RELATIVE comparison it dodges the CI-font trap. **A first red test passed while proving nothing**: its control string held a slash, so `Path` split it and the name under test collapsed to the last component - the pre-existing test's own comment warns of exactly that, and it was read and then repeated. Was: **NONE — the next action is the `P04` close itself (`/close-phase`, steps 5-9).** FP08's last seven shipped 2026-08-31, one commit each, all pushed (`97c0a7f`…`cf8608a`). **Four of the seven had a filed cause that measurement changed**, which keeps the standing rule earning its place: LWSM-1177's fix could not go where its evidence pointed (`_build_menus` runs before the strip exists, so the obvious placement broke 182 tests); LWSM-1180's item 5 needed no fix at all, having been corrected by LWSM-1165 weeks earlier; LWSM-1181 was filed as an ordering bug and is a forgery bug in all three fields, not one; LWSM-1191's route is narrower than filed, since Stop needs a port and so a never-port-less project cannot reach it. **LWSM-1178 and LWSM-1179 were filed unconfirmed and reproduced exactly**, so the rule is measure, not distrust. **A test caught a defect the LWSM-1191 fix introduced** — gating Start on the stop reservation left nothing to re-render the row when the stop finished, which is LWSM-1133's defect by another route; found by an assertion timing out, not by review. **Standing instruction from the user, 2026-08-31: file any issue found and not fixed immediately** — hence LWSM-1192/1193/1194 and the section holding them. Was: **`FP08` — next is LWSM-1176.** LWSM-1173 and LWSM-1175 shipped 2026-08-27 (`cf5e1d0`, `0730917`), leaving LWSM-1176…LWSM-1181 and LWSM-1191 as the seven still 📋. **LWSM-1173 reproduced exactly as filed** (a FIFO at the scan-roots path blocked until killed); its fix also had to CHOOSE a decode, since replacing the read forces one — `utf-8-sig`, matching the sibling reader of the same file, which closes LWSM-1182's class on the one reader that sweep could not reach because it was scoped to `read_bounded` consumers. Recorded there because a future reader will ask why the decode changed in a hardening item. **LWSM-1175 was filed NOT independently confirmed and the test was written first, before any fix — it reproduced on both `_rerender` callers.** That is the pattern the bullet itself prescribed ("one qtbot test settles it, and that test is worth writing whichever way it comes out") and it is cheaper than arguing: write the test, watch it, then design. Its over-correction — never clearing the error at all — is killed by a PRE-EXISTING test, so the behaviour is pinned from both sides. Previously: LWSM-1169 and LWSM-1170 shipped 2026-08-25 (`9d5b218`, `7a55ac7`). LWSM-1170's filed cause was CORRECTED by measurement, making that five consecutive FP08 bullets whose stated cause a reproduction changed — reproduce before designing, and treat a bullet's cause as a reading rather than a measurement (`CLAUDE.md` carries the trap). Previously: LWSM-1168 shipped 2026-08-25 (`599ea83`). Its supervisor half **reproduced**: a second real child spawned mid-stop, with the window held open at the `_on_wait` seam rather than raced for. Its UI half was **wrong in its stated mechanism** — `_settle_overlay` keeps a STOPPING overlay against a SIGTERM-ignoring child, and what re-enables Start early is LWSM-1133's port-less branch — so the residue is filed as **LWSM-1191** rather than folded in. Before FP08 resumed, the user-ordered detour shipped: **LWSM-1190** (a port read out of a quoted error message, which corrected MAME_Curator too — a second instance no fixture held) and **LWSM-1184** (a wrapper's program now gets the import walk; Contact_List detected, and RetroDB's 5000 now read from a declaration rather than guessed from Flask, so LWSM-1190's answer stopped being right by coincidence). **`confirm before implementing` is now three for three** — LWSM-1167, LWSM-1184 and LWSM-1168 each had a filed cause that measurement changed, and LWSM-1169 is the bullet flagged as most likely wrong. Was: **`FP08`, 13 items left — next is LWSM-1168.** LWSM-1167 shipped 2026-08-24 (`b594350`). **Six of the thirteen still rest on a review lane's quotation alone and say so on the bullet** — LWSM-1168, LWSM-1169, LWSM-1175, LWSM-1178, LWSM-1179 and three of LWSM-1180's five docstrings; LWSM-1169 is flagged as most likely wrong and LWSM-1179 as possibly not worth fixing. **Confirm before implementing**, which is what LWSM-1167 did and what turned its "fix shape" into a different, better fix. Was: **`FP08`, 14 items left — next is LWSM-1167, and it is CONFIRMED.** Its bullet carries a `CONFIRMED (2026-08-24)` block superseding its earlier "not independently confirmed" note: reproduced end to end, with the root cause and the fix shape written up on the bullet. Read `ADR-0004` before starting it — the ADR already specifies what the item asks for (`PortProbe` must answer *what holds the effective port?*, and `running (managed)` is *"effective port held by that child's group"*), and it carries two rules the bullet does not: the "looks like this project" test **may never be gated on**, and a managed server's identity is **the recorded child PID plus its `create_time`**, never the working directory. `LWSM-1011`'s seven-state classifier is the full implementation and `LWSM-1154` is blocked on it — LWSM-1167 is the foundation, not the whole thing. Was: **`FP08`, 5 of 21 shipped — next is LWSM-1166.** The four-lane fold-in from the P04 close, 2026-08-21: twenty findings, every one verified against the code before filing and three reproduced outright. **LWSM-1182 is the 21st and was filed on 2026-08-23 from inside the pass**, not by a lane. **Cadence, decided by the user 2026-08-23: all items in sequence, ONE AT A TIME** — red test, fix, `mutation_probe`, `local-ci.sh`, changelog, roadmap flip, one commit per item, push. Do not batch two items into one commit; the reason is that a mutation run measures one mechanism and a red gate must name one change. **Shipped 2026-08-23:** LWSM-1162 (launcher symlinked out of its project was not refused — `_contained` swallowed the escaping case, so ADR-0003's refusal was unreachable from `start()`), LWSM-1163 (one JSON typo plus one window close wrote defaults over every stored setting), LWSM-1164 (`RecursionError` is not a `ValueError`, so a nested settings file killed the app at startup), LWSM-1182 (a BOM refused `settings.json` and `scan-roots` where `registry`/`scanner` tolerate it — **taken out of order because LWSM-1163's write gate, shipped hours earlier, turned it from cosmetic into permanent**), LWSM-1165 (a child that exited on its own kept its slot for the session, so Start refused while Stop and Restart were greyed out). **Remaining, in order:** LWSM-1166 … LWSM-1181. LWSM-1181 is an investigate, not a confirmed defect. |
...
**Run the tool, do not read the code.** `LWSM-1304` was invisible to reading:
ruff, the types and every runtime test pass on a `translate()` call the
extractor silently ignores. One `pyside6-lupdate` run said "Found 0 source
text(s)" and the whole item followed from it. Same lesson as the `semgrep` and
stale-`.pyc` traps in CLAUDE.md.

**Three shell fixes needed a function extracted before they could be tested at
all** — `push_base()` in `.githooks/pre-push`, `tag_status()`, `dry_bump()` and
`release_triggers()` in `scripts/local-release.sh`. The precedent is
`docs_only()`: the test extracts the function body by regex and runs it under
`bash`, so it measures the decision rather than the text.
`scripts/local-release.sh` had NO tests before today.

**The backed-out `:disabled` style-sheet rule stays backed out, and this is not a reversal of that decision.** A rule there overrides the palette, which is why `muted_text` came out brighter than what it replaced. The layer was wrong, not the intent.

**Read the instrument lesson before styling any control state.** The filing measured a hand-built `QStyleOption`; the closure measured a real widget with no theme applied, where the stock Disabled group survives and dimming is genuine. Both were confident and neither described the running app. `CLAUDE.md`'s trap cluster carries the corrected form: render a real widget in the real state AND with the theme applied. The user's screenshot is what settled it — for anything whose value is how it looks, a picture from the reporter outranks every measurement here.

**The question that blocked this is answered and closed.** Detection was never at fault: the reported rows read `running` correctly. Do not re-open it as a port or `projects.json` problem.

**Then LWSM-1154 shipped and LWSM-1012 went 🚧, on the user's decision that the app must manage servers it did not start** — theirs come up at logon. Stop, Restart and Open are offered for any running row, each behind ADR-0004's disclosure; `service.py` drives the holder's systemd unit. **`disable` is absent from `service.VERBS` on purpose**, so start-at-logon cannot be regressed. LWSM-1301 carries the half not built: stopping a foreign server that is NOT a unit, which needs process-set enumeration and has a PID-reuse hazard the systemd route avoids.

**The recorded blocker on both items was false** — `RowView.managed` had answered foreign-vs-managed since LWSM-1141, so the seven-state classifier was never what they needed. Read a stated dependency before believing it.

**FP09 is the queue and is being worked highest-severity first**, not in order. LWSM-1261 and LWSM-1260 are done. Read the live figure, never one written here.

Earlier the same day: **LWSM-1248 shipped and pushed — `installed()` now resolves browsers through `mimeapps.list` rather than each entry's own `MimeType` claim.** Gate green, no SKIP, no tool drift. Two corrections from that item are worth carrying: a first version of its missing-home test SURVIVED all three guard mutants, because deleting `HOME` is not enough — `expanduser` falls back to the passwd entry, so patch `Path.home` as `test_applog.py` does. And **`Path.home()` RAISES rather than answering falsy**; reached as an XDG fallback inside `MainWindow.__init__` it aborted construction and left a window whose every later event failed on an attribute never assigned, surfacing as recursive Qt `changeEvent` tracebacks in unrelated font and geometry tests. **Two readings of mine were wrong and are corrected on the bullet**: `pytest-randomly` is NOT installed (the `pip list` saying so was reading another interpreter, so test order is deterministic), and a throwaway worktree's three HEAD failures were its own incomplete venv — HEAD was never broken. Before that, same day: **Twelve items shipped and pushed, one commit each; tree clean, nothing unpushed.** Ten FP09 items — LWSM-1239, 1240, 1241, 1242, 1243 (the whole `placement.py` cluster except LWSM-1238), then 1245, 1246, 1247, 1249, 1250 — plus two filed and fixed from live user reports the same day, LWSM-1297 and LWSM-1298. FP09 went 45/46 to **55 shipped / 36 active**; read the live figure, never this one. `local-ci.sh` green throughout, 1500 tests at the end, no SKIP, no tool drift; every fix mutation-probed and the mutants recorded on its bullet.

**Both of these are now ANSWERED — kept because each answer is worth more than the question was.** LWSM-1238 no longer needs a screenshot; see below. And **Rescan did populate Contact_List and RetroDB** (user, 2026-09-06), so the stale-file reading was right and there is NO merge defect — do not file one. The original note follows.

The user's own `projects.json` was STALE relative to the scanner: measured 2026-09-06 against the live `/mnt/Games/Scripts/Linux` tree, a fresh scan finds Contact_List on **5002** (`config.py`) and RetroDB on **5000** (`settings_manager.py`) where the app shows "no port", while MAME_Curator shows 1024 that a fresh scan does not find — which is the shape of a user `port_override`, preserved across rescans by design. **Rescan should fix the first two; if it does not, that is a merge defect worth an item.** Recorded here because it was diagnosed in chat and lives in no file otherwise.

**LWSM-1238 is UNBLOCKED as of 2026-09-06 and its scope has collapsed. Do not build the fork it describes.** Full measurements are on its own note; the three facts a session needs first are here.

**The app never loads Breeze.** PySide6 ships its own Qt, so the system Breeze plugin cannot bind to it — the resolvable styles are Windows and Fusion, on the venv interpreter and the system one alike. Every claim on LWSM-1238 and LWSM-1298 about "the reporter's Breeze desktop" is about a style this app does not use. Fusion IS the real style here, so the blocking observation costs a render rather than a screenshot.

**Fusion DOES draw a focus ring, in every palette.** "Offscreen Fusion draws nothing" was an instrument artifact: Qt gates focus indication on `State_KeyboardFocusChange`, and a render that omits it correctly draws nothing. A second artifact in the same pass sampled the button's interior, which cannot see a ring drawn at its border. Seven palettes clear the 3:1 non-text floor on the platform's own ring; **`graphite` alone is below it.** So what is left is one palette's colour, and the `QStyleSheetStyle` box-sizing hazard is not paid at all — no new border rule is needed. Re-scope before building.

**A changed-pixel count is not visibility, and this pair is the proof.** LWSM-1298 was filed and closed on "2175 of 2400 pixels change"; measured as contrast, that same rendering is 1.08:1 on `midnight` — invisible, every pixel having moved by a couple of RGB units. Measure a colour change as contrast, never as a population of moved pixels. The shipped pressed rule clears the floor in all eight palettes and was confirmed by rendering it, so **the user needed only to relaunch.**

**Four bullets were wrong about their own subject and each is corrected on its own note**: LWSM-1238's contrast figure (accent is 3.81:1 against ledger's `alt_base`, not 2.85, and every palette clears its floor — no new token needed); LWSM-1239's prescribed fix gave no account of the `x11` + `WAYLAND_DISPLAY` case it creates; LWSM-1242 misquotes the docstring it calls self-contradictory; and LWSM-1241 inherited a wrong API from ADR-0007 itself. **ADR-0007 specified `workspace.PlacementArea`, which is `undefined` in Plasma 6** — it worked only because `undefined` coerces to 0 and that happens to be the right enum. The real one is on the `KWin` global. ADR amended.

**A claim I filed myself was also wrong within the hour** (LWSM-1298: "any border rule re-boxes the widget") and is corrected in `theme.py` rather than left standing. Reproduce before designing, including your own bullets.

**`pgrep -af 'sleep' | grep -cE '(^| )sleep 30$'` gives FALSE POSITIVES** — it is machine-wide. One fired here and belonged to another Claude session polling CI in `Games_Hub`. Check the parent with `ps -o pid,ppid,lstart,args` before believing a hit; a real leak from this suite has been reparented to init. Recorded on LWSM-1189.

**Ants MCP: ANTS-4834 is FIXED and verified** — `apply_edits {dry_run:true}` now reports `would_write` / `would_apply` with the past-tense keys absent, and writes nothing. Marked completed in the feedback file, along with two new entries.) Was: 2026-09-03 (**Ten FP09 items shipped and pushed, one commit each; tip `5937a14`, tree clean, nothing unpushed.** LWSM-1227, 1228, 1230, 1231, 1232, 1233, 1234, 1235, 1236, 1237. `local-ci.sh` green throughout, 1454 tests at the end, no SKIP, no tool drift, no leaked processes; every fix mutation-probed and the mutants recorded on its bullet. **Four bullets were wrong or incomplete about their own fix** and each note says what was done instead — most sharply LWSM-1232, whose filed fix would have blinded the supervisor's port pre-flight to a real conflict, and LWSM-1233, whose first option does not work in Qt at all. Filed in passing: **LWSM-1295** (Stop and Restart are gated on the port table, so a machine that cannot read it can start servers it can never stop) and **LWSM-1296** (a mutation_probe timeout trap for CLAUDE.md). One correction: LWSM-1235's note and commit said 1451 tests where the gate said 1450; the note carries the fix and says the pushed commit still has the wrong number.) Was: 2026-09-02 (**Five FP09 items shipped and pushed; tip `07ed551`, working tree clean, nothing unpushed.** `local-ci.sh` green throughout, 1424 tests, no SKIP, every tool version matching `scripts/ci-tools.env`. **LWSM-1244 built Follow system** — a new core module `src/lwsm/appearance.py` (registered in `test_layering.py`'s `CORE_MODULES`), `theme.resolve_theme_id` + `FOLLOW_SYSTEM`, and two new `MainWindow` seams, `read_dark` and `read_high_contrast`. **Two measurements changed that design and are now traps in `CLAUDE.md`**: Qt 6.11.1 declares `Qt.ContrastPreference` and exposes NO `QStyleHints` accessor for it, so contrast comes from the XDG portal over `dbus-send`; and the offscreen platform IGNORES `QStyleHints.setColorScheme`, so the light/dark read HAD to become a seam or that half would be unverifiable. **LWSM-1254 ran a full `review-contract` gate** (genre `adr`, capped at 2 loops by `§ Review cadence`): 14 verified, 14 fixed, 1 dismissed. **Its run record is `docs/journal/P04.md`, NOT a loop-log section** — this document is user-facing, was split out of `design.md`, has never carried one, and its standard does not require it; do not add one. That gate also corrected `testing.md § T8` and `coding.md § O8`, both of which still forbade eliding outright. **Four questions the gate could not settle are filed as LWSM-1291–1294**, not left in the transcript. **`LocalWebServerManager_Ants_MCP_Feedback.md` was updated**: three earlier reports confirmed fixed by exercising them, and one new defect filed — `apply_edits` `dry_run:true` returns `files_written`/`edits_applied`, the past-tense-key defect ANTS-4463 already fixed on `roadmap_log`. Was: 2026-08-31 (**FP08 CLOSED — LWSM-1176, 1177, 1178, 1179, 1180, 1181 and 1191 shipped, one commit each, all pushed; tip `cf8608a`.** `local-ci.sh` green on every one, no SKIP, no tool drift, zero leaked processes before and after each run. Mutants killed per item: 1176 4/4, 1177 3/3, 1178 4/4, 1179 4/5, 1180 n/a (documentation-only, so no test — `testing.md § 8` and `write-test` both refuse one), 1181 4/4, 1191 7/7. **Two survivors, both recorded rather than cleared with a contrived fixture**: LWSM-1179's fifth is equivalent by construction (the guard raises before the mutated line runs), and LWSM-1177's fourth is a real pre-existing gap, now filed as LWSM-1192. **One process error worth not repeating**: `a211a3f`'s body states a test count that was never read — the gate was verified green, the number was not; corrected in `fedbb56`'s body rather than by rewriting a pushed commit. Quote no count you have not seen.) Was: 2026-08-25 (**LWSM-1169 and LWSM-1170 shipped and pushed, plus two traps folded into `CLAUDE.md`; tip `67863e6`.** Gate green on each — 1275 tests, no SKIP, no tool drift; mutants 4/4 killed on LWSM-1170, including one modelling the fix as the bullet prescribed it. Ants MCP ANTS-4629 confirmed fixed and marked in the shared feedback file.) Was: (**LWSM-1190, LWSM-1184 and LWSM-1168 shipped and pushed, tip `599ea83`.** Three items, three commits, gate green on each; LWSM-1191 filed from inside the last one. The instrument that earned its keep twice is the live-tree verdict diff — it found LWSM-1190's second instance and proved LWSM-1184 surgical, and no test count can say either. Was: 2026-08-24 (**LWSM-1167 shipped and pushed, tip `b594350`; FP08 down to 13.** `managed` is now derived from who holds the port rather than from our own records — the shape came from **ADR-0004**, not from the bullet, and that ADR carries two rules the bullet did not (the "looks like this project" test may never be gated on; managed identity is the child PID **plus `create_time`**). Six mutants killed, but the `_alive` PID-reuse guard SURVIVED the first pass because `stop()` pops the registry entry, so the test never reached the guard — the replacement uses a launcher that exits on its own. Filed on the way: **LWSM-1189** (two pre-existing tests leak a real `sleep 30` per run) and a new `CLAUDE.md` trap (stopping a child mid-start leaks its grandchild while `StopOutcome` reports success). Earlier the same day: **Three user-requested features shipped, tip `4351ffd`; FP08 otherwise untouched.** `LWSM-1186` version in the title bar. `LWSM-1187` a per-project browser picker in the row — which turned out to duplicate **`LWSM-1055`**, filed 2026-08-06 and found only afterwards, because Phase 0's `invariant_check` reads `docs/specs/` and cannot see a roadmap bullet; **run `task_priors` or `roadmap_query query=<topic>` before building a feature**. Finding it caught a real gap, now fixed: an uninstalled browser must fall back *with a visible message*. `LWSM-1174` was fixed because the picker could not ship without it — the row's controls were 7 px inside `design-accessibility.md`'s ~600 px magnifier band and the picker cost 84 px, so the name column is now capped and elided; the row ends at 561 px. `LWSM-1188` files what 1.0.0 means (the five success criteria + security + a packaged download; P09 and the debt sweep are explicitly OUT). `LWSM-1043` annotated with the finbreak auto-update research the user asked for — its own warning that "build an updater" is likely wrong is untouched, and it is blocked on `LWSM-1021` either way. 1233 green, local-ci green, 13 mutants killed. **Running the app found a bug 1233 tests could not**: accessible names were built from the elided label. Was: 2026-08-23 (**FP08 opened and five items shipped, each its own commit, all pushed; tip `a384ca9`.** 1162 green at the last gate, `local-ci.sh` green, 15 mutants killed across the five items. **One mutant survived and is recorded rather than papered over** — removing `with self._registry.lock` from `Supervisor.reap_exited` while keeping the identity check is equivalent-under-test, since no deterministic test can force that interleaving; the lock stays, because without it the identity check is itself a check-then-act. Also repaired seven doubled CHANGELOG headings in their own commit `99dd410`, and filed the tool-side cause with Ants MCP after reproducing it with `changelog_log dry_run`.) |
