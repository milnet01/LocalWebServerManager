...
**`FP07` is the active item and has not started.** Steps 5 and 6 of the parent
loop are ✅ — they are what generated it. **Two of the ten need a fixture class
that does not exist yet** (a project with `port=None`, and one per launcher
kind), so step 3 is larger here than the bullet count suggests.
