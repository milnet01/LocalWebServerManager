...
| **Last update** | 2026-09-06 (**LWSM-1300 REOPENED and SHIPPED. The original headline was right and both of the day's earlier readings were wrong.** The user asked for buttons irrelevant to a server's state to be disabled. `_apply_button_state` had always done that; nothing dimmed, so it read as broken. The fault was in `theme.to_palette`, whose two-argument `setColor` calls write one colour into the Active, Inactive AND Disabled groups alike — so applying a theme overwrote the platform's dimming. The Disabled group is now set for `WindowText`, `Text` and `ButtonText`.
