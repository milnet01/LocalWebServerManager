...
| **Last update** | 2026-09-06 (**LWSM-1300 filed, built, and BACKED OUT the same day — no behaviour changed, and the record is on its bullet.** The user asked for buttons irrelevant to a server's state to be disabled; `_apply_button_state` already does exactly that, and the platform already dims a disabled button visibly in all eight palettes. The finding rested on a hand-built `QStyleOption`, which does not reproduce the real disabled path — **the same broken instrument that produced LWSM-1238's "Fusion draws no focus ring" hours earlier**. A rule with eight-palette coverage and five killed mutants was written and reverted because rendering it showed it made the two states harder to tell apart. All four lessons are now in `CLAUDE.md`'s trap cluster; read them before styling any control state.

**ONE QUESTION IS OPEN AND IT BLOCKS THE USER'S ACTUAL REQUEST.** When a server is genuinely running, is Start still clickable? Two candidates, needing different fixes: they are on a build older than today's, or the app is not classifying the server as running. The second is live — their `projects.json` was stale with no port for two projects the same day, and **a running server whose port is unknown classifies as stopped**, which correctly offers Start and disables Stop while looking wrong. Ask before building anything.
